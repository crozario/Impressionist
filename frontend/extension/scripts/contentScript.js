...
let contentInfo = {
    allDialogueTimes : null, // format => [[startTime, endTime], ...]
    ...
    req.onreadystatechange = function () {
        if (req.readyState == 4) {
            if (req.status == 200) {  
                ...
                // contentInfo.characterNames = jsonObj.characters["characterNames"]
                // contentInfo.contentID = jsonObj.characters["_id"]
    ...
    req.onreadystatechange = function () {
        if (req.readyState == 4) {
            if (req.status == 200) {  
                ...
                console.log(req.responseText);
                let jsonObj = JSON.parse(req.responseText);
                contentInfo.gameID = jsonObj.gameID;
            } else {
...
let showUserSpeakElement = () => {
    let userSpeakElement = document.getElementById('user-speak');
    userSpeakElement.style.display = "block";
}

let hideUserSpeakElement = () => {
    let userSpeakElement = document.getElementById('user-speak');
    userSpeakElement.style.display = "none";
}
...
let goToPreviousDialogue = () => {
    if (contentInfo.previousDialogueID !== null) {
        const previousStartDialogue = contentInfo.allDialogueTimes[contentInfo.previousDialogueID][0];
...
let goToNextDialogue = () => {
    if (contentInfo.nextDialogueID !== null) {
        const nextStartDialogue = contentInfo.allDialogueTimes[contentInfo.nextDialogueID][0];
...
let updateDialogueID = () => {
    ...
    if (currentDialogueID !== null) {
        const currentStartDialogue = contentInfo.allDialogueTimes[currentDialogueID][0];
        const currentEndDialogue = contentInfo.allDialogueTimes[currentDialogueID][1];
        ...
        if (doesDialogueExist(contentInfo.currentDialogueID + 1, contentInfo.allDialogueTimes)) { // check if next dialogue exists
            const nextStartDialogue = contentInfo.allDialogueTimes[currentDialogueID + 1][0];
            const nextEndDialogue = contentInfo.allDialogueTimes[currentDialogueID + 1][1];
        ...
        if (contentInfo.allDialogueTimes.length > 0) {
            const firstStartDialogue = contentInfo.allDialogueTimes[0][0];
            const firstEndDialogue = contentInfo.allDialogueTimes[0][1];
            ...
            } else if (doesDialogueExist(contentInfo.previousDialogueID + 1, contentInfo.allDialogueTimes)) {
                const nextStartDialogue = contentInfo.allDialogueTimes[contentInfo.previousDialogueID + 1][0];
                const nextEndDialogue = contentInfo.allDialogueTimes[contentInfo.previousDialogueID + 1][1];
...
let checkAllDialogues = () => {
    ...
    for (const [currentDialogueID, currentDialogueArray] of contentInfo.allDialogueTimes.entries()) {
        if(isInBetweenDialogues(currentDialogueArray[0], currentDialogueArray[1])) {
            contentInfo.currentDialogueID = currentDialogueID;
            found = true;

            if (doesDialogueExist(currentDialogueID - 1, contentInfo.allDialogueTimes)) { // if previous exists
                contentInfo.previousDialogueID = currentDialogueID - 1;
            } else {
                contentInfo.previousDialogueID = null
            }
        
            if (doesDialogueExist(currentDialogueID + 1, contentInfo.allDialogueTimes)) { // if next exists
                contentInfo.nextDialogueID = currentDialogueID + 1;
            } else {
                contentInfo.nextDialogueID = null
            }
